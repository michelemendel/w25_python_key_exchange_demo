...
from aes_crypto import encrypt_message, decrypt_message, derive_aes_key
...
def run_demo():
    ...
    alice_aes_key = derive_aes_key(alice_shared_key)
    bob_aes_key = derive_aes_key(bob_shared_key)
    print(f"\n{alices} derived AES key (hex): {alice_aes_key.hex()}")
    print(f"{bobs} derived AES key (hex):   {bob_aes_key.hex()}")
    wait_for_enter()
    ...
    encrypted_message = encrypt_message(message, alice_aes_key)
    print(f"Encrypted message: {encrypted_message}")
    wait_for_enter()
    
    print_step(7)
    print(f"Bob receives the message, and decrypts it using his shared secret key")
    decrypted_message = decrypt_message(encrypted_message, bob_aes_key)
